...
function learnWords() {
  ...
  for (let i = 0; i < russianEnglishWords.length; i = i + 1) {
    // Это мой К, я его Комментирую чтобы сделать так, как делает Димы:
    // let rusWord = russianEnglishWords[i].russianWord;
    // let engWord = russianEnglishWords[i].englishWord;

    // Так - пишет К. Дима:

    settings[0].usingPrompt = prompt(russianEnglishWords[i].russianWord); //  changes
    if (settings[0].usingPrompt !== russianEnglishWords[i].englishWord) {
      //  changes
      alert(arrayMessages[1].messageIncorrect); // 'mistake'
      alert(settings[1].numberAnswersCorrect); // alert counter
      alert(settings[0].usingPrompt);
      alert(russianEnglishWords[i].englishWord); // changes
      // 5/04/24
      arrayWordsUnlearned.push(russianEnglishWords[i].englishWord);
      arrayAnswersIncorrect.push(settings[0].usingPrompt);
    } else {
      alert(arrayMessages[1].messageCorrect); // 'right'
      settings[1].numberAnswersCorrect++; // counter
      alert(settings[1].numberAnswersCorrect); // alert counter
    }
  }
